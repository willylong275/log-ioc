...
def main(df_hits, lock):
	...
	lock.acquire()
	lio.update_last_timestamp('powershell_execution', df_hits.iloc[(len(df_hits)-1)]['@timestamp'])
	#lio.update_last_timestamp('windows_account_creation', df_hits.iloc[(len(df_hits))]['@timestamp'])
	local_df=df_hits[(df_hits['@timestamp'] > lio.get_last_timestamp('powershell_execution')) & (df_hits['_source.event_id'] == 1)]
	lock.release()
	...
	if len(local_df) != 0:			
		for index, row in local_df.iterrows():
			...
			if "powershell.exe" in str(parent_image):
				message=str("command :"+command_line+" executed via powershell by user: "+user)
